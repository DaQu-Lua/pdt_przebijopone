			...
			if CanUseWeapon(allowedWeapons) then
				...
				if closestTire ~= nil then
					...
					if IsVehicleTyreBurst(vehicle, closestTire.tireIndex, 0) == false then
						Draw3DText(closestTire.bonePos.x, closestTire.bonePos.y, closestTire.bonePos.z, tostring("~b~Naciśnij~w~ ~r~[E]~w~ ~b~aby przebić oponę~w~"))
						if IsControlJustPressed(1, 38) then
							...
							if driverServer == 0 then
								...
								TriggerServerEvent("PDT:Przebijopone", driverServer, closestTire.tireIndex)
							end
							Citizen.Wait((animDuration / 2) * 1000)
...
RegisterNetEvent("PDT:Przebijopone2")
AddEventHandler("PDT:Przebijopone2", function(tireIndex)
	TriggerEvent("chatMessage", "^1Gracz postanowil przebic ci opone")
	local player = PlayerId()
	local plyPed = GetPlayerPed(player)
